...
This is will be site in which people without the mobile app can view a uPort identity.

## Current state

- Jan 24, 2017 - First pass at a simple rendering using our uPort libraries.

## Implementation

Bootstrapped with Facebook's Create-React-App tool. We intend to add Redux for proper management of data so the site can grow as needed.

## Etherum URL scheme

We are following the proposed [Ethereum URI EIP](https://github.com/ethereum/EIPs/issues/67)

```
ethereum:<address>[?value=<value>][?label=uPort%20Test%20App][?function=updateStatus(string%20%22I%27m%20happy%22)][?callback_url=http://testapp.uport.me/map/....]
```

### Auth request

We support a simplified oAuth2 request flow through both regular web and QR codes.

The format of the URL at the moment looks like this:

```
ethereum:me?callback_url=http://testapp.uport.me/map/....&[label=uPort%20Test%20App]
```

### Web based protocol handlers

- [Web based protocol handlers](https://developer.mozilla.org/en/docs/Web-based_protocol_handlers)
- [CanIUse Register Protocol Handler](http://caniuse.com/#feat=registerprotocolhandler)
...
Yarn is an alternative to npm that ensures that all the dependencies and the dependencies of the dependencies are the same for all developers.

The way it does this is through a file `yarn.lock` which is checked into github and is updated everytime we update dependencies.

Once dependencies are updated just type `yarn` instead of `npm install`

When updating a package use `yarn upgrade <package>`.

When adding a package use `yarn add <package>` instead of `npm install --save <package>`.
